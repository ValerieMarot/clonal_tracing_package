...
def VAF_plot(adata, sub_var=None, FC_sub_var=None, cov_sub_var=None, sub_cells=None, Cdiff_sub_cells=None, cell_sort=None, figsize=(7,6), y_ticks="chr", VAF="full", save_path=None):
    
    factor_ticks = [str(i) + " (" + adata.uns['factor_labels'][i]+")" for i in range(adata.varm["V"].shape[1])]
    print(factor_ticks)
    print(adata.varm["V"].shape[1])
    
    fig,((ax0, ax1), (ax2, ax3)) = plt.subplots(2, 2, gridspec_kw={'height_ratios':[1,.1], 'width_ratios':[.1,1]}, figsize=figsize)
...
def VAF_compare(adata, sub_var=None, FC_sub_var=None, cov_sub_var=None, figsize=(6,6), save_path=None):
    ...
    plt.hist(adata.varm["V"], color=[color_map[i] for i in adata.uns["factor_labels"]], label=adata.uns["factor_labels"])
    plt.legend()
    if save_path is not None:
        fig.savefig(save_path, bbox_inches='tight', format='pdf', dpi=300)
    plt.show()
